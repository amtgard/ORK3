/**
 * Search frontend flows (T-11 / DS-11 §2.3).
 */
...
test('search route loads', async ({ page }) => {
  await page.goto('./index.php?Route=Search');
...
test.describe('authenticated search flows', () => {
  ...
  test('universal search ajax responds after login', async ({ page }) => {
    const response = await page.goto('./index.php?Route=SearchAjax/universal&q=test');
    expect(response?.status()).toBeLessThan(500);
  });

  test('unit search route responds after login', async ({ page }) => {
    const response = await page.goto('./index.php?Route=Search/unitsearch');
    expect(response?.status()).toBeLessThan(500);
  });
});
