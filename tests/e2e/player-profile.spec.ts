/**
 * Player profile frontend flows (T-09 / DS-09 §2.3).
 */
...
test('player profile route loads', async ({ page }) => {
  await page.goto('./index.php?Route=Player/profile/1');
  await expect(page.locator('body')).toBeVisible();
});

test.describe('authenticated player flows', () => {
  ...
  test('player profile loads after login', async ({ page }) => {
    await page.goto('./index.php?Route=Player/profile/1');
    await expect(page.locator('body')).toBeVisible();
  });
});
