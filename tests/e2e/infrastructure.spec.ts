/**
 * Infrastructure frontend flows (T-13 / DS-13 §2.3).
 */
...
test('health route returns OK when DB is up', async ({ request, baseURL }) => {
  const res = await request.get(`${baseURL}/index.php?Route=Health`);
  expect(res.status()).toBeLessThan(500);
});

test.describe('authenticated infrastructure flows', () => {
  ...
  test('home route loads after login', async ({ page }) => {
