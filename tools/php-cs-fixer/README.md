...
```sh
php tools/php-cs-fixer/php-cs-fixer.phar fix                  # format the whole project
php tools/php-cs-fixer/php-cs-fixer.phar fix --dry-run -v     # show what WOULD change, write nothing
php tools/php-cs-fixer/php-cs-fixer.phar fix path/to/File.php # format specific files
```

### Upgrading the pinned version

```sh
curl -sL -o tools/php-cs-fixer/php-cs-fixer.phar \
  https://github.com/PHP-CS-Fixer/PHP-CS-Fixer/releases/download/vX.Y.Z/php-cs-fixer.phar
chmod +x tools/php-cs-fixer/php-cs-fixer.phar
```
