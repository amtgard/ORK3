...
class StateOfAmtgard
{
    ...
    public function getPlayerStats(string $start, string $end, array $kingdom_ids): array
    {
        ...
        $days_in_range = 365.0;
        if ($rs && $rs->Next()) {
            $days_in_range = (float)$rs->days_in_range;
        }
        if ($days_in_range <= 0) {
            $days_in_range = 365.0;
        }
        ...
        if ($rs) {
            while ($rs->Next()) {
                $trend_by_year[] = ['year' => (int)$rs->yr, 'sign_ins' => (int)$rs->cnt];
            }
        }
        ...
        if ($rs) {
            while ($rs->Next()) {
                $trend_normal_by_year[] = ['year' => (int)$rs->yr, 'normal_players' => (int)$rs->cnt];
            }
        }
        ...
        if ($rs && $rs->Next()) {
            $ten_avg_lifespan = (float)$rs->avg_lifespan;
        }
        ...
        if ($rs && $rs->Next()) {
            $ten_norm_lifespan = (float)$rs->avg_lifespan;
        }
    ...
    public function RangeLimitMonths(): int
    {
        return (getenv('ENVIRONMENT') === 'DEV') ? 0 : 12;
    }

    /**
     * @return array{ok: bool, start?: string, end?: string, httpCode?: int, error?: string}
     */
    public function ValidateDateRange(string $startInput, string $endInput, ?string $environment = null): array
    {
    ...
    /**
     * @return array{Kingdoms: array, MinDate: string, MaxDate: string, RangeLimitMonths: int}
     */
    public function GetPageBootstrap(): array
    {
        global $DB;
        $DB->Clear();
        $rng = $DB->DataSet(
            "SELECT MIN(date) AS mn, MAX(date) AS mx FROM " . DB_PREFIX . "attendance WHERE date BETWEEN '1990-01-01' AND CURDATE()"
        );
        $minDate = '';
        $maxDate = '';
        if ($rng && $rng->Next()) {
            $minDate = (string) $rng->mn;
            $maxDate = (string) $rng->mx;
        }

        return [
            'Kingdoms' => $this->getActiveKingdoms(),
            'MinDate' => $minDate,
            'MaxDate' => $maxDate,
            'RangeLimitMonths' => $this->RangeLimitMonths(),
        ];
    }

    /**
     * @param list<int> $kingdomIds
     * @return array<string, mixed>|null
     */
    public function DispatchChartSection(string $section, string $start, string $end, array $kingdomIds): ?array
    {
        switch (trim($section)) {
            case 'kingdoms':
                return ['kingdoms' => $this->getKingdomSignIns($start, $end, $kingdomIds)];
            case 'classes':
                return ['classes' => $this->getClassSignIns($start, $end, $kingdomIds)];
            case 'parks':
                return ['parks' => $this->getParksAnalysis($start, $end, $kingdomIds)];
            case 'players':
                return ['players' => $this->getPlayerStats($start, $end, $kingdomIds)];
            case 'cohorts':
                return ['cohorts' => $this->getPlayerCohorts($start, $end, $kingdomIds)];
            case 'longevity':
                return ['longevity' => $this->getPlayerLongevity($start, $end, $kingdomIds)];
            case 'retention':
                return ['retention' => $this->getNewPlayerRetention($kingdomIds)];
            case 'awards':
                return ['awards' => $this->getAwardGrants($start, $end, $kingdomIds)];
            default:
                return null;
        }
    }
