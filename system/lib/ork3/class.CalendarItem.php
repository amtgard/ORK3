...
class CalendarItem extends Ork3
{
    ...
    public function CreateCalendarItem($request)
    {
        ...
        if (!strlen($name)) {
            return InvalidParameter(null, 'Name is required.');
        }

        $allDay = !empty($request['AllDay']) ? 1 : 0;
        [$start, $end] = $this->normalizeDates($request['EventStart'] ?? '', $request['EventEnd'] ?? '', $allDay);
        if (!$start || !$end) {
            return InvalidParameter(null, 'Start and end dates are required.');
        }
        ...
        $this->item->color       = $this->normalizeColor($request['Color'] ?? '');
    ...
    public function UpdateCalendarItem($request)
    {
        ...
        if (!valid_id($id)) {
            return InvalidParameter(null, 'Invalid calendar item id.');
        }

        $this->item->clear();
        $this->item->calendar_item_id = $id;
        if (!$this->item->find()) {
            return InvalidParameter(null, 'Calendar item not found.');
        }
    ...
    public function GetCalendarItem($request)
    {
        ...
        return [
            ...
            'Color'          => $this->normalizeColor($this->item->color ?? ''),
            'Status'         => Success(),
        ];
    }

    // Calendar item colors are stored as #rrggbb. Accept any valid 6-digit hex
    // (the UI offers a fixed palette but a freeform hex is harmless), otherwise
    // fall back to the default slate.
    ...
    // Pick a readable text color (dark or white) for a given #rrggbb background using
    // perceived luminance — so a solid-color pill/bar stays legible on light hues (yellow)
    // and dark ones alike. Shared by the kingdom/park list + calendar render paths.
    ...
    public static function IsOfficer($mundane_id, $kingdom_id, $park_id)
    {
        ...
        if (Ork3::$Lib->authorization->HasAuthority($mundane_id, AUTH_ADMIN, 0, AUTH_CREATE)) {
            return true;
        }
        global $DB;
        $DB->Clear();
    ...
    {
        if ($is_officer_only && !self::IsOfficer($mundane_id, $kingdom_id, $park_id)) {
            return false;
        }
        if ($is_locals_only  && !self::IsLocal($mundane_id, $kingdom_id, $park_id)) {
            return false;
        }
        return true;
    }
