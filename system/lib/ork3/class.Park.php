...
class Park  extends Ork3 {
		...
		if ($r !== false && $r->size() > 0) {
			...
			do {
				$response['Parks'][] = array(
						...
						'Title' => $r->title,
						'Class' => $r->class,
						'ParentOf' => ($r->is_principality==1 && !array_search($r->park_id, $request['Stack']))?$this->GetParks(array('ParkId'=>$r->park_id, 'Stack' => push_stack($request['Stack'], $r->park_id) ) ):null
					);
			} while ($r->next());
		} else {
	...
	public function GetParkShortInfo($request) {
		...
		if ($this->park->find()) {
			...
			$k = Ork3::$Lib->kingdom->GetKingdomShortInfo(array('KingdomId'=>$this->park->kingdom_id));
	...
	public static function CalculateNextParkDay($recurrence, $week_of_month, $month_day, $week_day, $from_date = null) {
		...
		switch ($recurrence) {
			case 'weekly': return date("Y-m-d", strtotime("next $week_day", $from_date));
			case 'week-of-month':
					switch ($week_of_month) {
						case 1: return date("Y-m-d", strtotime("first $week_day of " . date("F Y", $from_date), $from_date));
						case 2: return date("Y-m-d", strtotime("second $week_day of " . date("F Y", $from_date), $from_date));
						case 3: return date("Y-m-d", strtotime("third $week_day of " . date("F Y", $from_date), $from_date));
						case 4: return date("Y-m-d", strtotime("fourth $week_day of " . date("F Y", $from_date), $from_date));
						case 5: return date("Y-m-d", strtotime("fifth $week_day of " . date("F Y", $from_date), $from_date));
					}
			case 'monthly': return date("Y-m-d", strtotime(date("F $month_day, $Y", $from_date), $from_date));
		}
	}
