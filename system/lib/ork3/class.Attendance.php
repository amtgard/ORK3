...
class Attendance extends Ork3
{
    ...
    public function GetAdjacentParkDates($request)
    {
        ...
        $this->db->Clear();
        $r = $this->db->DataSet(
            "(SELECT 'prev' AS direction, DATE(date) AS att_date FROM " . DB_PREFIX . 'attendance'
            . ' WHERE park_id = ' . $parkId . " AND date < '" . $date . "' ORDER BY date DESC LIMIT 1)"
            . ' UNION ALL '
            . "(SELECT 'next' AS direction, DATE(date) AS att_date FROM " . DB_PREFIX . 'attendance'
            . ' WHERE park_id = ' . $parkId . " AND date > '" . $date . "' ORDER BY date ASC LIMIT 1)"
        );
