  ...
  public function CrownQualed($kingdom_id) {
	...
	public function PlayerAwardRecommendations($request) {
		...
		$sql = "select 
			  ...
			  r.deleted_at,
			  r.deleted_by,
			  (SELECT COUNT(sub.awards_id) from " . DB_PREFIX . "awards sub WHERE sub.mundane_id = r.mundane_id AND (ka.kingdomaward_id = sub.kingdomaward_id OR (r.award_id > 0 AND a.award_id = r.award_id)) AND ((r.rank > 0 AND sub.rank >= r.rank) OR r.rank = 0 ))  as hasAward
