      ...
      logtrace("Tournaments", $sql);
        ...
        public function Custom($request) {
          // die(json_encode($request));
            ...
            } else if (strlen($filters['KingdomId']) > 0 && $filters['KingdomId'] > 0) {
                $location = " and m.kingdom_id = '" . mysql_real_escape_string($filters['KingdomId']) . "'";
                $duesclause = "a.kingdom_id = '" . mysql_real_escape_string($filters['KingdomId']) . "'";
                if (valid_id($request['ByKingdom'])) {
                    $park_list = Ork3::$Lib->Kingdom->GetParks($request);
                    $parks = array();
                    foreach ($park_list['Parks'] as $p => $park) {
            ...
            if ($filters['KingdomId'] > 0 || $filters['ParkId'] > 0) {
                if ($filters['isDuesPaid'])
                    $has_dues = "and s.is_dues = 1";
                            $duespaid_clause = "
                                            left join
                                                    (select distinct case split_id when null then 0 else 1 end as split_id, src_mundane_id
