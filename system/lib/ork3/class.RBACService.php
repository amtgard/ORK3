...
class RBACService extends Ork3
{
    ...
    public function EditRole($editor_id, $role_id, $permission_keys, $display_name = null, $description = null, $acting_kingdom_id = 0)
    {
        ...
        if ($acting_kingdom_id > 0 && (int) $role->kingdom_id !== $acting_kingdom_id && !$this->IsAdmin($editor_id)) {
            return NoAuthorization('That role belongs to another kingdom.');
        }
    ...
    /**
     * Registry permission definitions grantable at a given scope type. The scope
     * rule lives here (and in PermissionRegistry) so every consumer -- UI, API --
