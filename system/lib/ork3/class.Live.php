...
{
    public const CACHE_TTL_STATS  = 60;   // seconds; matches the client poll interval
    public const CACHE_TTL_RECENT = 20;   // ticker polls faster than the stats panel
    ...
    public function recent()
    {
        ...
        if ($rs && $rs->Size() > 0) {
            while ($rs->Next()) {
                ...
                if ((int)$rs->park_id > 0) {
                    $park_ids[(int)$rs->park_id]   = 1;
                }
                if ((int)$rs->event_id > 0) {
                    $event_ids[(int)$rs->event_id] = 1;
                }
            }
        }
