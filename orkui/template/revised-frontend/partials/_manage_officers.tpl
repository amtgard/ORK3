...
(function() {
	...
	// The Add form's office picker -- the only place this tab needs moData.
	function crPopulateOfficeSelect() {
		...
		all.slice().sort(function(a, b) {
			...
			opts += '<option value="' + pid + '">' + esc(label) + '</option>';
		});
		sel.innerHTML = opts;
		...
		keys.forEach(function(key) {
			...
			html += '<details class="cr-panel" open>' +
				'<summary class="cr-summary">' +
					...
					'<span class="cr-summary-title">' + esc(g.label) + '</span>' +
	...
	function crRowHtml(row) {
		...
		var personaHtml = mid > 0
			? '<a href="' + UIR + 'Player/profile/' + mid + '" class="cr-persona-link">' + esc(row.Persona || 'Unknown') + '</a>'
		...
		var termHtml = crIsOpenTerm(row)
			? ((start ? esc(start) : 'Unknown start') + ' &ndash; present')
			: ((start ? esc(start) : 'Unknown start') + ' &ndash; ' + (end ? esc(end) : 'unknown end'));
		var noteHtml = row.Notes ? ('<div class="cr-row-note">' + esc(row.Notes) + '</div>') : '';
	...
	function crEditRowHtml(row) {
		...
		return '<div class="cr-row cr-row-editing" data-hid="' + hid +
			'" data-orig-start="' + escAttr(crIsoOrEmpty(row.StartDate)) +
			'" data-orig-end="' + escAttr(crIsoOrEmpty(row.EndDate)) + '">' +
			...
			'<div class="ka-field">' +
				...
				'<textarea id="cr-edit-note" rows="2" maxlength="500">' + esc(row.Notes || '') + '</textarea>' +
			'</div>' +
	...
	window.crSaveEdit = function(hid) {
		...
		try {
			...
			moPost('edithistory', data, function() {
				crEditingId = 0;
				crLoad();
			}, function() {
				btn.disabled = false;
			...
			btn.disabled = false;
			btn.innerHTML = original;
			moShowNotice('Error', 'Could not save this term. Please try again.');
		}
	};

	window.crDelete = function(hid) {
		moShowConfirm('Delete This Term?',
			'This removes the record permanently. The audit log keeps a copy.',
			'Delete',
			function() {
				moPost('deletehistory', { OfficerHistoryId: hid }, function() {
					moCloseConfirm();
	...
	window.crAdd = function() {
		...
		try {
			...
			btn.disabled = false;
			btn.innerHTML = original;
			moShowNotice('Error', 'Could not add this term. Please try again.');
		}
	};
