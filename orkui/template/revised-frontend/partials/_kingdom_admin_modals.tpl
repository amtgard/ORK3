...
<?php
/* The .ka-* modal chrome (form rows, choice cards, merge preview, heraldry,
   confirm-dialog variants). Extracted verbatim to partials/_ka_modal_chrome.tpl
   so the Park console wears the same chrome instead of a second copy. Still a
   body <style> block, emitted at exactly this point, so the rules that beat
   admin-console.css on source order go on beating it. */
...
(function() {
	...
	/* ── Shared modal engine ──────────────────────────
	   The generic half of this file -- the overlay stack, the focus trap, the
	   dirty guard, the save-or-discard prompt and kaConfirm -- now lives in
	   partials/_ka_modal_core.tpl, included just above, so the Park admin console
	   can drive the same modals without a second, drifting copy. Nothing about its
	   behaviour changed: these aliases exist so the handlers below go on reading
	   exactly as they did when the engine was defined inline.

	   kaOpenModal / kaCloseModal are not aliased: the markup above calls them
	   straight from inline onclick, i.e. off window, which is where the engine
	   still exports them. */
	var kaRegisterModal     = window.kaRegisterModal;
	var kaIgnoreDirtyFields = window.kaIgnoreDirtyFields;
	var kaConfirm           = window.kaConfirm;
	var kaFeedback          = window.kaFeedback;
	var kaClearFeedback     = window.kaClearFeedback;
	var kaMarkClean         = window.kaMarkClean;
	/* Same signature the engine used to expose directly, so the four call sites
	...
	kaIgnoreDirtyFields(['ka-alias-search', 'ka-awards-search', 'ka-parks-search']);

	/* ── This console's modals ────────────────────────
	   Modals that must never reopen carrying an abandoned selection. The array is
	   the submit buttons to re-disable on open (they are enabled only once a pick
	   is made); `true` means reset the fields and disable nothing. */
	...
	kaRegisterModal('ka-config-overlay',       { resetOnOpen: true });
	...
	(function() {
		...
		/* Every control stamps defaultValue / defaultChecked / defaultSelected as well
		   as the live value. This modal's kaRegisterModal({resetOnOpen}) entry restores
		   it from those defaults, so they have to carry the STORED setting rather than
		   a blank. */
	...
	(function() {
		...
		function makeAwardRow(aw) {
			...
			/* Shared by the row's own Save button AND the bulk "Save Changes" action on
			   the unsaved-changes prompt (the kaRegisterModal save hook below) -- one
			   save request per row, built on the same kaPost() every other save in this
			   file uses, never reimplemented. cb(ok, message) fires exactly once.
		...
		kaRegisterModal('ka-awards-overlay', { save: function(done) {
			var dirty = [];
			groups.forEach(function(g) {
