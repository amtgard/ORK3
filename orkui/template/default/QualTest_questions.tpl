...
<div class="rp-root">
	...
	<div class="rp-body">
		...
		<div class="rp-table-area">
					...
					<tr id="qrow-<?= $q['QualQuestionId'] ?>">
						...
						<td>
							<div class="qt-actions-cell">
								...
								<button class="qt-action-btn qt-action-btn-archive qt-status-btn"
								        data-id="<?= $q['QualQuestionId'] ?>"
								        data-kingdom="<?= $KingdomId ?>"
								        data-status="archived">
									<i class="fas fa-archive"></i> Archive
								</button>
								<button class="qt-action-btn qt-action-btn-reset qt-reset-btn" title="Reset Success Rate"
								        data-id="<?= $q['QualQuestionId'] ?>"
								        data-kingdom="<?= $KingdomId ?>">
									<i class="fas fa-sync-alt"></i>
								</button>
								<?php if ($q['ReportCount'] > 0): ?>
								<button class="qt-flag-btn qt-report-flag-btn" title="<?= $q['ReportCount'] ?> report<?= $q['ReportCount'] !== 1 ? 's' : '' ?>"
								        data-id="<?= $q['QualQuestionId'] ?>"
								        data-kingdom="<?= $KingdomId ?>">
			...
			<!-- Archived questions -->
			<div class="rp-table-section-title" style="margin-top:32px;">
				Archived Questions
					...
					<tr id="qrow-<?= $q['QualQuestionId'] ?>">
						...
						<td>
							<div class="qt-actions-cell">
								...
								<button class="qt-action-btn qt-action-btn-restore qt-status-btn"
								        data-id="<?= $q['QualQuestionId'] ?>"
								        data-kingdom="<?= $KingdomId ?>"
								        data-status="active">
...
<div class="qt-report-overlay" id="qt-library-overlay">
	<div class="qt-report-modal" style="max-width:720px;width:95%;max-height:85vh;display:flex;flex-direction:column;">
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-shrink:0;">
			...
			<button id="qt-library-close" style="background:none;border:none;font-size:1.4rem;cursor:pointer;color:#718096;line-height:1;">&times;</button>
		</div>
...
$(function() {
	if ($('#qt-active-table tbody tr').length > 0) {
		$('#qt-active-table').DataTable({ pageLength: 25, order: [[2, 'desc']] });
	}
	if ($('#qt-archived-table tbody tr').length > 0) {
		$('#qt-archived-table').DataTable({ pageLength: 25, order: [[2, 'desc']] });
	}
		...
		btn.addEventListener('click', function() {
			...
			fetch('<?= UIR ?>QualTestAjax/resetstats', { method: 'POST', body: fd })
				...
				.then(function(j) {
					...
					// Clear the % Success cell (3rd td, index 2)
					var cells = row.querySelectorAll('td');
					if (cells[2]) cells[2].innerHTML = '<span class="qt-success-badge qt-success-none">—</span>';
				});
		});
	});
});
