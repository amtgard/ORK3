...
<?php 		endif; ?>
					...
					<td><?php if (valid_id($award['GivenById'])) : ?><a href='<?=UIR.'Player/index/'.$award['GivenById']?>'><?=htmlspecialchars($award['GivenBy'])?></a><?php else : ?><?=htmlspecialchars($award['GivenBy'] ?? '')?><?php endif; ?></td>
