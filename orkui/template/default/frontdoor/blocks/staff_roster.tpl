...
// Resolve the renderable roster BEFORE emitting anything. The consent gate below
// can drop a row entirely (a person with no persona name and no opt-in has no
// publishable name), so the heading must not be printed until we know at least
// one card survives — a published "Board of Directors" header over an empty grid
// reads as a kingdom with no leadership. Same bail as rich_text.tpl /
// kingdom_parks.tpl: nothing at all publicly, a hint for the author in preview.
$rosterCards = [];
foreach ((is_array($people) ? $people : []) as $person) {
    ...
    $rosterCards[] = [
        ...
        'photoAlt'  => (($img['alt'] ?? '') !== '') ? (string) $img['alt'] : $primary,
        'link'      => $link,
    ];
}

if (empty($rosterCards)) {
    if ($fdIsPreview) {
        fdEmptyBlockNotice('This staff roster has nobody to show yet.');
    }
    return;
}
?>
<div class="fd-pad fd-roster">
    ...
    <div class="fd-roster-grid">
        <?php foreach ($rosterCards as $card): ?>
            <?php
            ...
            <?= $open ?>
                <?php if ($photoSrc !== ''): ?>
                    <img class="fd-roster-photo" src="<?= htmlspecialchars($photoSrc, ENT_QUOTES) ?>" alt="<?= htmlspecialchars($card['photoAlt'], ENT_QUOTES) ?>">
                <?php else: ?>
