...
class Model_Unit extends Model
{
    ...
    /**
     * Unit authorizations go through AuthorizationGate so unit.auth.manage actually
     * grants -- calling AddAuthorization directly reaches only the legacy authority
     * rule. The gate still falls back to that rule (KPM roster bypass included) for
     * anyone who does not hold the permission.
     */
    public function add_unit_auth($request)
    {
        logtrace("add_unit_auth()", $request);
        return (new AuthorizationGate())->GrantScopedAuthorization($request);
    }
    ...
    public function del_unit_auth($request)
    {
        ...
        return (new AuthorizationGate())->RevokeScopedAuthorization($request);
    }
