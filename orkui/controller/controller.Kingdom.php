...
class Controller_Kingdom extends Controller
{
    ...
    public function profile($kingdom_id = null)
    {
        ...
        $this->data['OrgUnitLabel']            = $this->Kingdom->GetOrgUnitLabel($kingdom_id, false);
        $this->data['OrgUnitLabelPlural']      = $this->Kingdom->GetOrgUnitLabel($kingdom_id, true);
        // Distinct child terms (for the Parks-tab grouping of THIS kingdom's child principalities).
        $this->data['ChildOrgUnitLabelPlural'] = $this->Kingdom->GetChildOrgUnitLabel($kingdom_id, true);
        // Term map for the admin dropdown JS (lib Kingdom class is loaded via the model).
        $this->data['OrgUnitTerms'] = class_exists('Kingdom') ? Kingdom::ORG_UNIT_TERMS : [];
            ...
            foreach ($this->data['principalities']['Principalities'] as $pr) {
                ...
                if (!empty($prMapParks)) {
                    $this->data['prinz_map_parks'][] = [
                        'KingdomId' => $prId,
                        'Name'      => $prName,
                        'prTerm'    => $this->Kingdom->GetOrgUnitLabel($prId, false),
                        'parks'     => $prMapParks,
                    ];
                }
            }
