...
class Controller_SelfReg extends Controller
{
    ...
    public function form($token = null)
    {
        ...
        // Already-logged-in short-circuit. A self-reg link is for people who
        // DON'T have an account yet; letting a signed-in player submit the
        // form would create a duplicate account under a fresh username.
        // Surface a friendly "you're already registered" page with a link to
        // their profile instead of the form. Token still validated so an
        // already-expired link gets the "invalid" page consistently.
        if (isset($this->session->user_id) && (int)$this->session->user_id > 0) {
            $this->data['token']            = $token;
            $this->data['already_logged_in'] = true;
            $this->data['current_user'] = [
                'mundane_id'   => (int)$this->session->user_id,
                'persona'      => (string)($this->session->user_name ?? ''),
                'park_name'    => (string)($this->session->park_name ?? ''),
                'kingdom_name' => (string)($this->session->kingdom_name ?? ''),
            ];
            $_pl = $this->Player->fetch_player((int)$this->session->user_id);
            if (is_array($_pl) && !empty($_pl['Persona'])) {
                $this->data['current_user']['persona'] = (string)$_pl['Persona'];
            }
            return;
        }
